...
contract Auction {
    ...
    modifier onlyAuctioneer{
        require(msg.sender == auctioneer, "It's not auctioneer");
        _;
    }
    modifier InStates(State _state){
        require(state == _state, "Invalid State!");
        _;
    }
    modifier onlyBid(address _account){
        require(msg.sender == _account, "Not the same current Bidder");
        _;
    }
    ...
    // Start the session.
    function startSession() public InStates(State.CREATED) onlyAuctioneer{
        state = State.STARTED;
    }
    

    function bid(uint8 _price) public InStates(State.STARTED){
    ...
    function getDeposit() public InStates(State.CLOSING){
